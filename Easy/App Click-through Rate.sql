...
(
(SELECT app_id, count(*) as impression FROM events WHERE event_type='impression' AND EXTRACT(YEAR from timestamp)='2022' GROUP BY app_id) a
INNER JOIN
(SELECT app_id, count(*) as click FROM events WHERE event_type='click' AND EXTRACT(YEAR from timestamp)='2022' GROUP BY app_id) b
