...
SELECT age_bucket,
  ...
  FROM
  (
    (SELECT * FROM activities WHERE activity_type IN ('open', 'send')) a
    JOIN
    (SELECT * FROM age_breakdown) b
...
SELECT age_bucket, 
ROUND((send_time_spent/(send_time_spent+open_time_spent))*100,2) AS send_perc,
ROUND((open_time_spent/(send_time_spent+open_time_spent))*100,2) AS open_perc
from temp;